        ...
        {newArrivalProducts.map((product, index) => (
          <div key={index} className="relative group hover:lg:hovered">
            ...
            <div className="mt-4 text-center">
              <h4 className="font-opensans text-[14px] md:text-[18px]">{product.title}</h4>
              <p className="font-opensans text-[14px] md:text-[18px]">&#8358;{product.price}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="text-center mt-12">
        <button className="tracking-[.2rem] z-10 md:text-[15px] text-[12px] h-[50px] md:h-[64px] px-[2rem] md:px-[3rem] mt-4 font-opensans font-semibold bg-dark text-secondary">VIEW ALL</button>
      </div>
    </div>
  );
};
